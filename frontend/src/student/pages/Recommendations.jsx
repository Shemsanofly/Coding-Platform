    ...
    <div className="space-y-6 overflow-x-hidden p-4 pb-24 md:pb-6 md:p-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-ink dark:text-sand">Your focus areas</h1>
          <p className="mt-1 text-sm text-muted dark:text-muted">
            Course and lesson context for each recommendation.
          </p>
        </div>
        <button
          type="button"
          onClick={refreshAll}
          disabled={recsQuery.isFetching || weaknessesQuery.isFetching}
        ...
        >
          {recsQuery.isFetching || weaknessesQuery.isFetching ? "Refreshing…" : "Refresh"}
        </button>
      </header>
      ...
      {showLoading ? <LoadingState label="Analyzing your weak topics…" rows={4} /> : null}

      {!showLoading && (recsQuery.isError || weaknessesQuery.isError) ? (
        ...
        <>
          {!weaknesses.length && !items.length ? (
            <EmptyState
              ...
              message="Complete at least one quiz so the system can analyze your learning needs."
            />
          ) : null}
          ...
          {focusCards.length ? (
            <section className="space-y-4">
              <SectionHeader
                title="Focus areas"
                subtitle="Each card ties a weak topic to the lesson where it showed up and what to study next."
              />
            ...
            <EmptyState
              ...
              message="No matching lesson was found yet. Ask your instructor to add lessons tagged with these topics."
            />
          ) : null}
        </>
      ) : null}
