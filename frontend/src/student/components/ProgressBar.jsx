    ...
    <div className={className}>
      {(label || showPercent) && (
        <div className="mb-1.5 flex items-center justify-between gap-2 text-xs text-muted dark:text-muted">
          ...
          {showPercent ? (
            <span className="font-semibold tabular-nums text-ocean-700 dark:text-reef">
              {Math.round(safe)}%
            </span>
          ) : null}
        </div>
      ...
      <div
        className={`w-full overflow-hidden rounded-full bg-reef/50 dark:bg-ocean-950/60 ${height}`}
      >
        <div
