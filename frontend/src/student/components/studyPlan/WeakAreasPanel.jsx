...
export default function WeakAreasPanel({ courseFilter, onCourseFilterChange }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <article className="rounded-2xl border border-ocean-600/10 bg-white p-4 shadow-sm dark:border-line/40 dark:bg-ocean-950/50">
          <p className="text-sm text-muted dark:text-muted">Total weak topics</p>
          <p className="mt-2 text-2xl font-bold text-ink dark:text-sand">{stats.totalWeakTopics}</p>
        </article>
        <article className="rounded-2xl border border-red-200/70 bg-white p-4 shadow-sm dark:border-red-400/30 dark:bg-ocean-950/50">
          <p className="text-sm text-muted dark:text-muted">High severity</p>
          <p className="mt-2 text-2xl font-bold text-ink dark:text-sand">{stats.highCount}</p>
        </article>
        <article className="rounded-2xl border border-amber-200/70 bg-white p-4 shadow-sm dark:border-amber-400/30 dark:bg-ocean-950/50">
          <p className="text-sm text-muted dark:text-muted">Medium severity</p>
      ...
      {!showLoading && isError ? (
        <ErrorState message="Could not load weak topics." onRetry={() => void refetch()} />
      ) : null}

      {!showLoading && !isError && !lessonGroups.length && !topics.length ? (
        <EmptyState
      ...
      {!showLoading && !isError && groupedByCourse.length > 0 ? (
      ...
      {!showLoading && !isError && topics.length > 0 && !lessonGroups.length ? (
