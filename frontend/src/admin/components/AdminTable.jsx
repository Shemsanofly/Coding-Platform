    ...
    <>
      <div className="hidden overflow-x-auto md:block">
        <table className="min-w-full divide-y divide-line">
          <thead className="bg-reef/60 dark:bg-ocean-950/60">
            <tr>
              {columns.map((col) => (
                <th
                  key={col.key}
          ...
          <tbody className="divide-y divide-line/70 bg-white dark:bg-ocean-950/30">
            {rows.map((row) => (
              <tr key={row.id} className="transition hover:bg-reef/40 dark:hover:bg-ocean-900/40">
                {columns.map((col) => (
