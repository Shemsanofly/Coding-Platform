...
class ProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('first_name', 'last_name', 'experience_level', 'profile_image', 'remove_profile_image')
        extra_kwargs = {
    ...
    def validate(self, attrs):
        user = self.instance
        if user.role == User.Role.STUDENT and 'experience_level' in attrs and not attrs.get('experience_level'):
            raise serializers.ValidationError(
                {'experience_level': 'Learning level is required for student accounts.'}
            )
        return attrs
