...
class LearningPathServiceTests(TestCase):
    ...
    def test_gemini_explanation_generation_mocked(self, mock_client_cls):
        mock_client_cls.return_value.models.generate_content.return_value = type(
            "R",
            (),
            {"text": "Focus on Classes before Inheritance because inheritance builds on classes."},
        )()

        with patch(
            "ai_engine.services.learning_path.config",
            side_effect=lambda key, default="": "key" if key == "GEMINI_API_KEY" else default,
        ):
