...
class WeaknessContextAPITests(TestCase):
    def setUp(self):
        ...
        self.intro_quiz = Quiz.objects.create(lesson=self.intro_lesson, generation_status=Quiz.GenerationStatus.DONE)
        ...
        self.summary_url = reverse("student-lesson-weakness-summary", kwargs={"lesson_id": self.intro_lesson.id})

    def test_weakness_api_includes_course_context(self):
        self.client.force_authenticate(user=self.student)
        response = self.client.get(self.weakness_url)
        self.assertEqual(response.status_code, 200)
        row = next(item for item in response.data["topics"] if item["topic_tag"] == "java_variables")
    ...
    def test_student_cannot_filter_by_unenrolled_course(self):
        ...
        recommendation_response = self.client.get(self.recommendation_url, {"course_id": unenrolled.id})
    ...
    def test_lesson_weakness_summary_without_quiz_returns_message(self):
        self.client.force_authenticate(user=self.student)
        url = reverse("student-lesson-weakness-summary", kwargs={"lesson_id": self.practice_lesson.id})
