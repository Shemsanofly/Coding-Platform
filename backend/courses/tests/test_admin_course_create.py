...
def test_admin_can_create_course_with_draft_status():
    admin = User.objects.create_user(email="admin-create@example.com", password="pass", role="admin")
    client = APIClient()
...
def test_student_cannot_create_course():
    student = User.objects.create_user(email="student-create@example.com", password="pass", role="student")
    client = APIClient()
